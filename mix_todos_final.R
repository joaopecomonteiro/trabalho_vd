  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      conditionalPanel(
        ...
        dateRangeInput("date_range", "Select Date Range:", start = min(data$Created.Date), end = max(data$Created.Date)),
        sliderInput("top_complaints", "Number of Top Complaint Types:", min = 5, max = 50, value = 10),
        checkboxInput(inputId = "log_filter", label = "Log Filter", value = FALSE),
        conditionalPanel(
          condition = "input.log_filter == true",
          sliderInput(inputId = "log_range_filter", label = "Filter by Log Range (min difference):", min = 0, max = 5, value = 0, step = 0.1)
        ...
        checkboxGroupInput("borough", "Select Boroughs:", 
                           choices = unique(data$Borough), 
                           selected = unique(data$Borough)),
        checkboxGroupInput("period_filter", "Select the day period:", 
                           choices = c("Morning", "Afternoon", "Night", "Early Morning"), 
                           selected = c("Morning", "Afternoon", "Night", "Early Morning")),
        checkboxGroupInput("weekday_filter", "Select the day of the week:", 
      ...
      ################################ JOAO
      conditionalPanel(
        condition = "input.tab_selector == 'Map'",
        dateRangeInput("date_range", "Date Range:",
                       start = min(data$Created.Date, na.rm = TRUE),
                       end = max(data$Created.Date, na.rm = TRUE),
                       format = "yyyy-mm-dd "),
        ...
        radioButtons(
          ...
          label = "Color Type:",  # Display label in the UI
          choices = c("Complaint Type", "Agency", "Day Type"),  # List of unique complaint types
          selected = "Complaint Type"  # Pre-select all by default
        ),
        
        radioButtons(
          inputId = "selected_day_type", 
          label = "Day Type:",  # Display label in the UI
          choices = c("None", "Week Day", "Weekend Day and Holiday"),  # List of unique complaint types
          selected = "None"  # Pre-select all by default
        ),
        
        
        sliderInput("duration_filter", "Duration:",
                    min = min(data$Resolution.Time, na.rm = TRUE),
                    max = round(max(data$Resolution.Time, na.rm = TRUE)),
                    value = c(min(data$Resolution.Time, na.rm = TRUE),
                              round(max(data$Resolution.Time, na.rm = TRUE)))),
        
        numericInput("min_Duration", "Minimum Duration:", 
                     min = min(data$Resolution.Time, na.rm = TRUE),
                     max = round(max(data$Resolution.Time, na.rm = TRUE)),
                     value = min(data$Resolution.Time, na.rm = TRUE)),
        
        numericInput("max_Duration", "Maximum Duration:", 
                     min = min(data$Resolution.Time, na.rm = TRUE),
                     max = round(max(data$Resolution.Time, na.rm = TRUE)),
        ...
        radioButtons(
          inputId = "select_complaint_type_filter", 
          label = "Complaint Type Filter:",  # Display label in the UI
          choices = c("Top n", "Complaint Type"),  # List of unique complaint types
          selected = "Top n"  # Pre-select all by default
        ),
        
        selectInput(
          inputId = "selected_complaint_type",  
          label = "Complaint Type:",  
          choices = sort(unique(data$Complaint.Type.Clean)),  
          selected = sort(unique(data$Complaint.Type.Clean))[1]
        ),
        
        selectInput(
          inputId = "selected_agency",  
          label = "Agency:",  
          choices = c("None", sort(unique(data$Agency))),  
          selected = "None"
        )
...
server <- function(input, output, session) {
  ...
  filtered_data_carol <- reactive({
    ...
    # Filter for Borough
    if (length(input$borough) < length(unique(data$Borough))) {
      data_filtered_carol <- data_filtered_carol %>% filter(Borough %in% input$borough)
    }
    
    # Filter by Period of Day
    if (length(input$period_filter) < 4) {
      data_filtered_carol <- data_filtered_carol %>% filter(Period.Of.Day %in% input$period_filter)
    }
    
    # Filter by Weekday
    if (length(input$weekday_filter) < 7) {
      data_filtered_carol <- data_filtered_carol %>% filter(weekdays(Created.Date) %in% input$weekday_filter)
    }
  ...
  observe({
    updateCheckboxGroupInput(
      session,
      "complaint_type",
      choices = top_complaints(),
      selected = top_complaints()
    )
  })
  ...
  output$heatmap <- renderPlotly({
    heatmap_data <- filtered_data_carol() %>%
      ...
      summarise(
        ...
        Average.Log.Resolution.Time = mean(log1p(Resolution.Time), na.rm = TRUE),
        .groups = 'drop'
      )
    ...
    heatmap_plot <- plot_ly(
      ...
      type = "heatmap",
      colorscale = "Viridis",
      text = ~paste("Hour:", Hour, "<br>Day:", Weekday, "<br>Value:", round(get(z_col), 2)),
      hoverinfo = "text",
      colorbar = list(title = colorbar_title)  # Dynamic legend
    ) %>%
      layout(
        title = if (input$log_filter) "Log-Average Resolution Time by Hour and Day" else "Average Resolution Time by Hour and Day",
  ...
  output$line_chart <- renderPlotly({
    ...
    line_plot <- plot_ly(
      ...
      layout(
        ...
        xaxis = list(title = "Date", tickformat = "%d-%m", tickangle = 45),
        yaxis = list(title = "Average Time (hours)")
      )
    line_plot
  })
  ...
  output$polar_chart <- renderPlotly({
    ...
    polar_plot <- plot_ly(
      ...
      r = ~Count,
      theta = ~Hour * 15,
      text = ~paste("Hour:", Hour, "<br>Frequency:", Count),
      hoverinfo = "text",
      marker = list(color = ~Count, colorscale = "Viridis")
    ) %>%
      layout(
        title = "Frequency of Complaints by Hour of Day",
        polar = list(
          angularaxis = list(
          ...
          radialaxis = list(title = "Frequency")
        )
      )
    polar_plot
  })
  
  # Frequency by Period of Day
  output$bar_chart <- renderPlotly({
    bar_data <- filtered_data_carol() %>%
      filter(!is.na(Resolution.Time)) %>%
      mutate(Period.Of.Day = factor(Period.Of.Day, levels = c("Early Morning", "Morning", "Afternoon", "Night"))) %>%
    ...
    bar_plot <- plot_ly(
      ...
      marker = list(line = list(color = 'rgba(58, 71, 80, 1)', width = 0))  
    ) %>%
      layout(
        title = "Frequency of Complaints by Time of Day",
        xaxis = list(title = "Period of the Day"),
  ...
  output$stacked_bar <- renderPlotly({
    top_complaints <- filtered_data_carol() %>%
      count(Complaint.Type.Clean, sort = TRUE) %>%
      arrange(desc(n)) %>%
      head(input$top_complaints) %>%
      pull(Complaint.Type.Clean)
    ...
    stacked_bar_plot <- ggplot(complaint_data, aes(x = Weekday, y = Volume, fill = Complaint.Type.Clean)) +
      ...
      scale_fill_viridis_d(name = "Complaint Type") +
      labs(
        title = "Complaint Types by Day of the Week",
        x = "Day of the Week",
        y = "Volume"
      ) +
  ...
  ################################## JOAO
  observe({
    # Update slider when numeric inputs are changed
    updateSliderInput(session, "duration_filter", value = c(input$min_Duration, input$max_Duration))
  })
  
  observe({
    # Update numeric inputs when slider is changed
    updateNumericInput(session, "min_Duration", value = input$duration_filter[1])
    updateNumericInput(session, "max_Duration", value = input$duration_filter[2])
  })
  
  unique_complaint_count <- reactive({
    length(unique(data$Complaint.Type.Clean))
  })
  
  # Dynamically update the slider's max value
  observe({
    updateSliderInput(
      session,
  ...
  filter_by_duration <- reactive({
    
    weekend_dates <- as.Date(c("2016-9-3", "2016-9-4", "2016-9-5", 
                               "2016-9-10", "2016-9-11", "2016-9-17", 
                               "2016-9-18", "2016-9-24", "2016-9-25"))
    ...
    data_j <- data_j %>%
      mutate(day_type = ifelse(as.Date(Created.Date) %in% weekend_dates, 
                               "weekend", "weekday"))
    
    if (input$select_complaint_type_filter == "Top n"){
      ...
      filtered_data <- data_j %>%
        filter(Complaint.Type.Clean %in% top_complaints$Complaint.Type.Clean)
    }
    
    else {
      filtered_data <- data_j %>%
        filter(Complaint.Type.Clean %in% input$selected_complaint_type)
    }
    ...
    filtered_data <- filtered_data %>%
      filter(
        Resolution.Time >= input$duration_filter[1], 
        Resolution.Time <= input$duration_filter[2]   
      )
    
    if (input$selected_agency != "None") {
      filtered_data <- filtered_data %>%
        filter(
          Agency == input$selected_agency   
        )
    }
    
    if (input$selected_day_type == "Weekend Day and Holiday") {
      filtered_data <- filtered_data %>%
        filter(
          day_type == "weekend"   
        )
    ...
    else if (input$selected_day_type == "Week Day") {
      filtered_data <- filtered_data %>%
        filter(
          day_type == "weekday"   
        )
    ...
    filtered_data <- filtered_data %>%
      sample_frac(input$sample_size / 100)
    
    
    return(filtered_data)
  })
  
  output$Joao_map <- renderPlotly({
    plot_data <- filter_by_duration()
    
    
    alpha = 0.1
    
    if (nrow(plot_data) == 0) {
      # If no data, plot just the empty map
      p <- ggplot() +
      ...
      if (input$selected_color_type == "Complaint Type") {
        # If there is data, plot the complaints
      ...
      else if (input$selected_color_type == "Agency") {
        p <- ggplot(data = plot_data) +
          geom_sf(data = nyc_boroughs, fill = "gray90", color = "black") +
      ...
      else if (input$selected_color_type == "Day Type") {
        p <- ggplot(data = plot_data) +
          geom_sf(data = nyc_boroughs, fill = "gray90", color = "black") +
